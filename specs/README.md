...
> **規約の正本は作者のワークスペースにある `SPEC-OPS.md`(このリポジトリには含まない)。** ディレクトリ構成・frontmatter・GitHub 規約・
...
複製時に置換する: `{{FEATURE_SLUG}}` `{{FEATURE_TITLE}}` `{{RELEASE}}` `{{PRIORITY}}` `{{TEST}}`

`{{TEST}}` だけは sed で置換しない — テストコマンドは `/` と `&` を含みうるので sed が壊れる。ファイルを開いて書き換える。
